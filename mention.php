...
class qa_html_theme_layer extends qa_html_theme_base
{
	...
	//	$this->output_raw($content);
		$mentionedcontent = preg_replace("/@([A-Za-z0-9_]{1,15})/", " <a href=\"/user/$1\">@$1</a>", $content);
		if (strlen($content))
...
//		$this->output_raw($a_item['content']);
        $acontent = preg_replace("/@([A-Za-z0-9_]{1,15})/", " <a href=\"/user/$1\">@$1</a>", $a_item['content']);
		if (strlen($acontent))
			echo $acontent;
		$this->output('</div>');
	}	
	...
	//	$this->output_raw($c_item['content']);
	    $ccontent = preg_replace("/@([A-Za-z0-9_]{1,15})/", " <a href=\"/user/$1\">@$1</a>", $c_item['content']);
		if (strlen($ccontent))
			echo $ccontent;
		$this->output('</div>');
	}
